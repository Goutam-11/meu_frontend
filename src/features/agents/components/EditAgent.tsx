...
export default function EditAgentPage({
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      id: agent?.id,
      name: agent?.name,
      strategy: agent?.strategy ?? AgentStrategy.LLM_TRADING,
      type: agent?.type ?? AgentType.CRYPTO,
      llmModel: agent?.llmModel ?? MODELS[0],
      credentialId: agent?.credentialId ?? "",
      temperature: agent?.temperature ?? 0.7,
      exchangeId: agent?.exchangeId ?? "",
      market: {
        symbols: agent?.market?.symbols.map(s => ({ value: s })) ?? [{ value: "" }],
        agentCycles: agent?.market?.agentCycles ? formatSecondsToCycle(agent?.market?.agentCycles) : "1h",
      },
      risk: {
        maxRiskPerTradePct: agent?.risk?.maxRiskPerTradePct ?? 1,
        maxDailyLossPct: agent?.risk?.maxDailyLossPct ?? 5,
        maxOpenPositions: agent?.risk?.maxOpenPositions ?? 3,
      },
      capital: {
        allocated: agent?.capital?.allocated ?? 100,
        currency: agent?.capital?.currency ?? "USDT",
      },
    },
  });
  ...
  return (
    <Card className="bg-background h-full w-full border-none shadow-none overflow-auto max-h-full">
      <CardHeader>
        <CardTitle className="text-2xl">ReConfigure Trading Agent to Edit</CardTitle>
        <CardDescription>
      ...
      <CardContent>
        <div className="mb-4">
          <p className="text-sm border rounded-2xl border-accent-foreground p-4">Agent ID: {agent?.id}</p>
        </div>
        <Form {...form}>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormDescription>
                        The exchange will be used to fetch market data and trade.
                      </FormDescription>
                      <FormMessage />
                    </FormItem>
                  )}
